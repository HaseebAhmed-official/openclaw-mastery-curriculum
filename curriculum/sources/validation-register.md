...
| VR-007 | MCP `2026-07-28` uses host/client/server roles, stateless requests, per-request protocol/capability metadata, and optional up-front `server/discover`; protocol support alone does not prove safe authorization or correct behavior. | verified-primary | MCP `2026-07-28` architecture/base protocol; final clause is engineering inference. Older initialization-era revisions differ. | MCP specification or SDK protocol-era release |
| VR-008 | A2A `1.0.0` represents agent discovery plus task/message/artifact exchange across versioned bindings; compatibility and trust require binding/auth tests. | verified-primary | A2A latest released specification; curriculum adds test requirement. | A2A specification release |
| VR-009 | Shared observability semantics improve portability, but GenAI conventions must be version-pinned and now come from the dedicated OpenTelemetry GenAI repository rather than the core repository. | verified-primary | OpenTelemetry core and GenAI semantic-convention repositories. | Convention migration, stability, or version change |
